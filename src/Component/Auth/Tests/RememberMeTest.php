...
class RememberMeTest extends \PHPUnit_Framework_TestCase
{
	...
	public function setUp()
	{
		$this->key = 'EeJoowaprM';
		$this->username = 'aName';
		$this->cookie = $this->getMockBuilder('Pagekit\Component\Cookie\CookieJar')
							->disableOriginalConstructor()
							->getMock();
		$this->provider = $this->getMockBuilder('Pagekit\Component\Auth\UserProviderInterface')
							->disableOriginalConstructor()
							->getMock();
	...
	public function testAutologin()
	{
		$this->provider->expects($this->exactly(1))
					...
					->will($this->returnCallback(function() {
						$user = $this->getMock('Pagekit\Component\Auth\UserInterface');
		...
		$this->assertInstanceOf('Pagekit\Component\Auth\UserInterface', $this->remember->autoLogin($this->provider));
	}

	/**
     * @expectedException        Pagekit\Component\Auth\Exception\AuthException
	...
	public function testAutoLoginInvalidHashException()
	{
		$this->provider->expects($this->exactly(1))
				->method('findByUsername')
				->will($this->returnCallback(function() {
					return $user = $this->getMock('Pagekit\Component\Auth\UserInterface');
				}));
	...
	public function testSet()
	{
		...
		$user = $this->getMockBuilder('Pagekit\Component\Auth\UserInterface')
						->disableOriginalConstructor()
						->getMock();
