...
class FilterManager implements \IteratorAggregate
{
    ...
    {
        if (!is_string($filter) || !is_subclass_of($filter, 'Pagekit\Component\Menu\Filter\FilterIterator')) {
            throw new \InvalidArgumentException(sprintf('Given filter "%s" is not of type Pagekit\Component\Menu\Filter\FilterIterator', (string) $filter));
        }
